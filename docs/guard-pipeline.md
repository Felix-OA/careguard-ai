...
The Stage 3 dashboard reads only the public event representation through the Audit API aggregation layer. It replaces request content with a fixed summary, removes excerpts and protected-response references, preserves rejected/admitted/refill distinctions, and keeps proposed/authorized/blocked/failed/executed tool arrays separate. Browser filters and pagination are bounded server-side.
