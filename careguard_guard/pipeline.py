...
from careguard_guard.models import (
    Decision, GuardChatRequest, GuardChatResponse, GuardMode, SecurityEvent,
)
...
def _public_sources(raw: list[SourceMetadata]) -> list[SourceMetadata]:
    return [item.model_copy(update={"excerpt": item.excerpt if item.admitted_to_context else None}) for item in raw]
        ...
        try:
            if mode == GuardMode.ENFORCE and request_enforcement == Decision.BLOCK:
                ...
                if self.config.enabled_controls.get("retrieval_guard", True):
                    admitted_docs, raw_metadata, rejected_metadata, would_admitted, refilled = guard_retrieval(
                    ...
                    rejected_metadata, would_admitted, refilled = [], list(raw_metadata), False
                ...
                if mode == GuardMode.MONITOR:
                    ...
                    if self.config.enabled_controls.get("tool_guard", True):
                        preview = self.tool_guard.evaluate(proposed, request, execute=False)
                    ...
                    if self.config.enabled_controls.get("tool_guard", True):
                        ...
                        if blocked_tools:
                            would_decisions.append(Decision.BLOCK)
                        if confirmation_status in {"required", "missing", "expired", "changed_action"}:
                            would_decisions.append(Decision.REQUIRE_CONFIRMATION)
                    else:
                        proposed = target_response.proposed_tool_calls
                        authorized = proposed
