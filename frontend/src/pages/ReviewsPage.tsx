...
export function ReviewsPage() {
  ...
  return <><PageHeader eyebrow="Human judgment" title="Review queue" description="Automated results remain immutable. Reviewer decisions and notes are stored as a separate local demonstration workflow." /><div className="review-summary">{(['UNREVIEWED','CONFIRMED_SAFE','CONFIRMED_FINDING','NEEDS_MORE_CONTEXT'] as ReviewStatus[]).map(value => <button key={value} className={filter === value ? 'active' : ''} onClick={() => setFilter(value)}><StatusBadge value={value} /><strong>{currentItems.filter(item => item.decision.status === value).length}</strong></button>)}</div>
    ...
    {items.length ? <div className="review-list">{items.map(item => <Card key={item.review_id} className="review-card"><div className="review-meta"><span>{item.source_type.replace('_',' ')}</span><span>{new Date(item.timestamp).toLocaleString()}</span><StatusBadge value={item.automated_result} />{item.is_stale && <StatusBadge value="historical" />}</div><h2>{item.objective_id ?? item.scenario_id ?? item.source_id}</h2>{item.is_stale && <p className="field-help">Superseded by run {item.superseded_by}; excluded from the current unresolved count.</p>}<p className="review-reason">{item.review_reason}</p><p className="evidence-quote">{item.evidence_summary}</p>{item.campaign_id && item.objective_run_id && <p><a href={`/agentic/${item.campaign_id}/objectives/${item.objective_run_id}`}>Inspect sanitized trajectory →</a></p>}<div className="tag-list">{item.policy_categories.map(value => <span key={value}>{value}</span>)}</div><div className="review-footer"><StatusBadge value={item.decision.status} /><button className="button" onClick={event => { reviewTriggerRef.current = event.currentTarget; setEditing(item); setStatus(item.decision.status); setNote(item.decision.note ?? '') }}>Record review</button></div></Card>)}</div> : <EmptyState title="No review items in this state">Change the status filter or run the guarded suite.</EmptyState>}
    {editing && <div className="modal-backdrop" role="presentation" onMouseDown={e => e.target === e.currentTarget && setEditing(null)}><section ref={dialogRef} className="modal" role="dialog" aria-modal="true" aria-labelledby="review-dialog-title"><div className="card-heading"><h2 id="review-dialog-title">Review {editing.objective_id ?? editing.scenario_id ?? editing.source_id}</h2><button className="icon-button" aria-label="Close review dialog" onClick={() => setEditing(null)}>×</button></div><div className="alert alert-review"><strong>Automated result: {editing.automated_result}</strong><span>This decision will not overwrite the automated evidence.</span></div><label>Reviewer decision<select autoFocus value={status} onChange={e => setStatus(e.target.value as ReviewStatus)}><option>UNREVIEWED</option><option>CONFIRMED_SAFE</option><option>CONFIRMED_FINDING</option><option>NEEDS_MORE_CONTEXT</option></select></label><label>Concise reviewer note<textarea maxLength={500} value={note} onChange={e => setNote(e.target.value)} /></label>{save.error && <div className="alert alert-error" role="alert">{save.error.message}</div>}<div className="button-row"><button className="button" onClick={() => setEditing(null)}>Cancel</button><button className="button button-primary" onClick={() => save.mutate()} disabled={save.isPending}>Save review</button></div></section></div>}
  </>
}
